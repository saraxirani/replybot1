...
async function main() {
    ...
    try {
        ...
        // 5. Find the first tweet that hasn't been replied to and reply
        for (const tweet of searchResult.data.data) {
            ...
            // Found a new tweet to reply to
            const randomReply = replyTexts[Math.floor(Math.random() * replyTexts.length)];
            console.log(`Attempting to reply to new tweet ID: ${tweet.id} with: "${randomReply}"`);
